...
public class MarketServiceImpl implements MarketService {
    ...
    @Override
    public RealTimeEntity lastYearData(MarketVo marketVo) {
        //去年今天
        String endTime = timeUtils.getNowOfLastYear(marketVo.getStartTime(),-1);
        //去年第一天
        String startTime = timeUtils.getLastYearFirstDay(marketVo.getStartTime(),-1);
        marketVo.setStartTime(startTime);
        marketVo.setEndTime(endTime);
        marketVo.setDateType(Constants.D);

        return marketDao.lastYearData(marketVo);
    }

    @Override
    public String queryMarketPlanSale(MarketVo marketVo) {
        return marketDao.queryMarketPlanSale(marketVo);
    }

    @Override
    public List<MarketRealSaleAmountEntity> realTimeSaleAmount(MallVo mallVo) {
        RequestEntity requestEntity = new RequestEntity();
        requestEntity.setGroupId(mallVo.getGroupId());
        requestEntity.setMarketId(mallVo.getMarketId());
        requestEntity.setMallId(mallVo.getMallId());
        requestEntity.setFloorId(mallVo.getFloorId()); //为地图数据的楼层

        String URL = urlHeader+ "112";

        return httpUtil.httpUtil(URL,requestEntity,MarketRealSaleAmountEntity.class);
