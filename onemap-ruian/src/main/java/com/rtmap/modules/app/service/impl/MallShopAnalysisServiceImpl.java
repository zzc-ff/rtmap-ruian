...
public class MallShopAnalysisServiceImpl extends ServiceImpl<MallDao, ShopRankEntity> implements MallShopAnalysisService {
    ...
    public R bostonShopList(ParameterVo parameterVo) {
        ...
        double saleAmountMedian = 0.0;
        double saleEffectMedian = 0.0;
        if (list != null && list.size()!= 0) {
            ...
            saleAmountMedian = mathUtils.getMedian(saleAmountData);
            saleEffectMedian = mathUtils.getMedian(saleEffectData);
        }else {
            log.info("波士顿图获取数据失败！");
        }
